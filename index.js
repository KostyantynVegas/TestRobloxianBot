...
bot.on('message', message => {
    if (message.content == ("/help")) {
        message.reply("Russian:/Rhelp. English:/Ehelp.");
    }
});
bot.on('message', message => {
    if (message.content == ("/Rhelp")) {
        message.reply("Скоро будет адаптация под русский!");
    }
});
...
bot.on('message', message => {
    if (message.content == ("/Ehelp")) {
        message.reply("credits, message, avatar, meme, puzzle.");
    }
});
//credits.
bot.on('message', message => {
...
//random puzzle.
bot.on('message', message => {
    var p1 = "Sorry, this command dont works. :anguished: ";
    if (message.content == (prefix +"puzzle")) {
        message.reply(p1);
    }
});
